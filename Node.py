...
class Node:
    ...
    def propose_samples_sobol(self, num_samples, path, lb, ub, problem, net):
        proposed_X, proposed_obj, X_X = self.classifier.propose_rand_samples_sobol(num_samples, path, lb, ub, problem, net=net)
        return proposed_X, proposed_obj, X_X
    ...
    def propose_samples_Bayesian_Optimization(self, num_samples, path, lb, ub, model, mll, train_obj, func, acq_func='qehvi', cur_iter=0, net=None):
        proposed_X, proposed_obj, X_X = self.classifier.propose_rand_samples_bayesian(num_samples, path, lb, ub, model, mll, train_obj, func, acq_func=acq_func, cur_iter=cur_iter, net=net)
        return proposed_X, proposed_obj, X_X
    ...
    def __str__(self):
        ...
        if torch.cuda.is_available():
            upper_bound = np.around(np.max(self.classifier.samples[2].cpu().data.numpy(), axis=0), decimals=2)
            lower_bound = np.around(np.min(self.classifier.samples[2].cpu().data.numpy(), axis=0), decimals=2)
        else:
            upper_bound = np.around(np.max(self.classifier.samples[2].data.numpy(), axis = 0), decimals=2 )
            lower_bound = np.around(np.min(self.classifier.samples[2].data.numpy(), axis = 0), decimals=2 )
        boundary    = ''
