...
class MW(Problem):
    ...
    def g1(self, X):
        ...
        z = torch.pow(X[:, self.n_obj - 1:], n)
        i = torch.arange(self.n_obj - 1, d)
    ...
    def g2(self, X):
        ...
        i = torch.arange(self.n_obj - 1, d)
        z = 1 - torch.exp(-10.0 * (X[:, self.n_obj - 1:] - i / n) * (X[:, self.n_obj - 1:] - i / n))
...
class MW1(MW):
    def __init__(self, n_var=15,n_obj=2, **kwargs):
    ...
    def _evaluate(self, X, out={}, *args, **kwargs):
        ...
        out["F"] = torch.column_stack([f0, f1])
    ...
    def _calc_pareto_front(self, ref_dirs=None):
        if ref_dirs is None:
            ...
            F = ref_dirs
        F[:, 1] = 1 - 0.85 * F[:, 0]
        l = torch.sqrt(2) * F[:, 1] - torch.sqrt(2) * F[:, 0]
        c = 1 - F[:, 0] - F[:, 1] + 0.5 * torch.sin(2 * torch.pi * l) ** 8
        F = F[c >= 0]
        return F
...
class MW5(MW):
    def __init__(self, n_var=15,n_obj=2, **kwargs):
        super().__init__(n_var, n_obj, 3)

    def _evaluate(self, X, out={}, *args, **kwargs):
        g = self.g1(X)
        f0 = g * X[:, 0]
        f1 = g * np.sqrt(1.0 - torch.pow(f0 / g, 2.0))

        with np.errstate(divide='ignore'):
            atan = np.arctan(f1 / f0)

        g0 = f0 ** 2 + f1 ** 2 - torch.pow(1.7 - self.LA2(0.2, 2.0, 1.0, 1.0, atan), 2.0)
        t = 0.5 * np.pi - 2 * np.abs(atan - 0.25 * np.pi)
        g1 = torch.pow(1 + self.LA2(0.5, 6.0, 3.0, 1.0, t), 2.0) - f0 ** 2 - f1 ** 2
        g2 = torch.pow(1 - self.LA2(0.45, 6.0, 3.0, 1.0, t), 2.0) - f0 ** 2 - f1 ** 2
        out["F"] = np.column_stack([f0, f1])
...
class MW6(MW):
    def __init__(self, n_var=15, n_obj=2, **kwargs):
        super().__init__(n_var, n_obj, 1, xl=0.0, xu=1.1)

    def _evaluate(self, X, out={}, *args, **kwargs):
        g = self.g2(X)
        f0 = g * X[:, 0]
        f1 = g * np.sqrt(1.1 * 1.1 - torch.pow(f0 / g, 2.0))

        with np.errstate(divide='ignore'):
            atan = np.arctan(f1 / f0)

        g0 = f0 ** 2 / torch.pow(1.0 + self.LA3(0.15, 6.0, 4.0, 10.0, atan), 2.0) + f1 ** 2 / torch.pow(
...
class MW7(MW):
    ...
    def _evaluate(self, X, out={}, *args, **kwargs):
        g = self.g3(X)
        f0 = g * X[:, 0]
        f1 = g * np.sqrt(1 - torch.pow(f0 / g, 2))

        with np.errstate(divide='ignore'):
            atan = np.arctan(f1 / f0)

        g0 = f0 ** 2 + f1 ** 2 - torch.pow(1.2 + np.abs(self.LA2(0.4, 4.0, 1.0, 16.0, atan)), 2.0)
        g1 = torch.pow(1.15 - self.LA2(0.2, 4.0, 1.0, 8.0, atan), 2.0) - f0 ** 2 - f1 ** 2
        out["F"] = np.column_stack([f0, f1])
        out["G"] = np.column_stack([g0, g1])
...
class MW11(MW):
    def __init__(self, n_var=15,n_obj=2, **kwargs):
        super().__init__(n_var, n_obj, 4, xl=0.0, xu=np.sqrt(2))

    def _evaluate(self, X, out={}, *args, **kwargs):
        g = self.g3(X)
        f0 = g * X[:, 0]
        f1 = g * np.sqrt(2.0 - torch.pow(f0 / g, 2.0))
...
class MW13(MW):
    def __init__(self, n_var=15,n_obj=2, **kwargs):
        super().__init__(n_var, n_obj, 2, xu=1.5)
