...
from botorch_lamcts.botorch.utils.multi_objective.box_decomposition import NondominatedPartitioning
...
class MCTS:
    ...
    def __init__(self, lb, ub, dims, ninits, func, args, run=0,init_samples=None):
        self.dims = dims
        self.samples = []
        self.nodes = []
        self.Cp = args.cp
    ...
    def init_surrogate_model(self):
        train_x = self.samples[0].cpu()
        train_obj = self.samples[1].cpu()
    ...
    def init_train(self, pro):
        problem = pro
        init_samples = self.init_samples[0]
        # draw_sobol_samples(bounds=problem.bounds, n=1, q=10, seed=torch.randint(600000, (1,)).item()).squeeze(0)
        init_obj = self.init_samples[1]
        # problem(init_samples)

        if torch.cuda.is_available():
            init_samples, init_obj = init_samples.cuda(), init_obj.cuda()


        self.samples.append(init_samples)
        self.samples.append(init_obj)
        self.samples.append(self.init_samples[2])
        self.random_samples.append(init_samples)
        self.random_samples.append(init_obj)
        self.random_samples.append(self.init_samples[2])
    ...
    def search(self, net):
        ...
        pareto_y = train_obj[pareto_mask].cpu()
        # if torch.cuda.is_available():
        ...
        for i in range(0, self.args.iter):
            ...
            for j in range(0, 1):
                ...
                if self.args.sample_method == 'bayesian':
                    ...
                    samples, objs, X_X = leaf.propose_samples_Bayesian_Optimization(sample_num, path, self.lb, self.ub, self.surrogate_model, self.mll, train_obj, self.func, cur_iter=i, net=net)
                    ...
                    samples, objs,X_X = leaf.propose_samples_sobol(sample_num, path, self.lb, self.ub, self.func, net)
                self.samples[0] = torch.cat([self.samples[0], X_X], axis=0)
                self.samples[1] = torch.cat([self.samples[1], objs.view(1, -1)], axis=0)
                self.samples[2] = torch.cat([self.samples[2], samples.view(1, -1).cpu()], axis=0)
...
#                 final_cands_idx = np.random.choice(len(selected_cands), sample_num)
                ...
                hv = botorch_hv.compute(pareto_y.cpu())
                hv_track.append(hv)
...
def mcts(f, init_samples, model):
    ...
    parser.add_argument('--iter', type=int, default=1, help='total iterations')
    parser.add_argument('--sample_num', type=int, default=1, help='sample numsbers per iteration')
    ...
    parser.add_argument('--sample_method', type=str, default='random', help='bayesian, cmaes or random')
    ...
    parser.add_argument('--node_select', type=str, default='mcts', help='mcts or leaf')
    ...
    if not torch.cuda.is_available():
        agent = MCTS(lb=f.bounds[:, 0].data.numpy(), ub=f.bounds[:, 1].data.numpy(), dims=f.dim, ninits=10, func=f,
                     args=args, run=args.data_id,init_samples=init_samples)
    else:
        agent = MCTS(lb=f.bounds[:, 0].cpu().data.numpy(), ub=f.bounds[:, 1].cpu().data.numpy(), dims=f.dim, ninits=10,
                     func=f, args=args,
                     run=args.data_id,init_samples=init_samples)
    new_preference = agent.search(model)
    return new_preference, agent
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sample_num', type=int, default=10, help='sample numsbers per iteration')
    parser.add_argument('--runs', type=int, default=5, help='total runs')
    parser.add_argument('--cp', type=float, default=6, help='cp value in MCTS')
    ...
    f =  DTLZ2(negate=True,dim=10).to(**tkwargs)
