...
class Classifier():
    ...
    def get_hypervolume(self, ref_point):
        ...
        pareto_y = Y[pareto_mask].cpu()
        hv = botorch_hv.compute(-pareto_y)

        return hv

    def learn_boundary(self, plabel):
        assert len(plabel) == len(self.samples[2])
    ...
    def is_splittable_svm(self):
        ...
        # x = self.samples[:, 0]
        if torch.cuda.is_available():
            x = self.samples[2].cpu().data.numpy()
        else:
            x = self.samples[2].data.numpy()
        ...
        for idx in range(0, len(plabel)):
            if plabel[idx] == 0:
                if len(good_samples) == 0:
                    data = self.samples[2][idx].reshape(1, -1)
                    obj = self.samples[1][idx].reshape(1, -1)
                    x = self.samples[0][idx].reshape(1, -1)
                    ...
                    good_samples[2] = torch.cat([good_samples[2], self.samples[2][idx].reshape(1, -1)])
            else:
                if len(bad_samples) == 0:
                    ...
                    bad_samples[2] = torch.cat([bad_samples[2], self.samples[2][idx].reshape(1, -1)])

        # print('bad sample is!', bad_samples[0])
        # print('length here is!', len(good_samples[0]), len(bad_samples[0]), len(self.samples[0]))


        assert len(good_samples[0]) + len(bad_samples[0]) == len(self.samples[2])
        return good_samples, bad_samples
    ...
    def viz_node_sample_region(self, nums_samples, path, lb, ub, iter):
        ...
        while len(selected_cands) <= nums_samples:
            ...
            cands = np.random.dirichlet([1] * problem.n_obj, 1200000).astype(np.float32)
            cands = torch.tensor(X_rnd)
    ...
    def propose_rand_samples_sobol(self, nums_samples, path, lb, ub, problem, net=None):
        #rejected sampling
        ...
        # path = path[:1]
        while len(selected_cands) <= nums_samples:
            cands = np.random.dirichlet([1] * problem.n_obj, 15000).astype(np.float32)
        ...
        X_X = net(new_x.to(torch.float32))[0]
        new_obj = problem._evaluate(X_X)
        return new_x.detach(), new_obj.detach(), X_X.detach()
    ...
    def propose_rand_samples_cmaes(self, num_samples, path, lb, ub, func, init_within_leaf, LEAF_SAMPLE_SIZE=5, vanilla=False, samples=None):
        ...
        sample_X = self.samples[2].cpu().numpy()
    ...
    def propose_rand_samples_bayesian(self, nums_samples, path, lb, ub, model, mll, train_obj, func, acq_func='qehvi', cur_iter=0, net=None):
        ...
        def optimize_qehvi_and_get_observation(model, train_obj, sampler, path, net):
            ...
            # optimize
            eq_constrain = [(torch.tensor([0, 1, 2]), torch.tensor([1, 1, 1]),1)]
            ...
            candidates, _ = optimize_acqf(
                ...
                lamcts_boundry=path,
                cur_iter=cur_iter,
                equality_constraints = eq_constrain,
                net=net,
            )
        ...
        if acq_func == 'qehvi':
            ...
            new_samples, new_obj, X_X = optimize_qehvi_and_get_observation(model, train_obj, sampler, path, net)
        elif acq_func == 'qparego':
            new_samples = optimize_qparego_and_get_observation(model, train_obj, sampler, path)


        return new_samples, new_obj, X_X
